--/
create or replace script EXA_DB_MIGRATION.BIGQUERY_TO_EXASOL(
    CONNECTION_NAME
...
-- This script maintains backward compatibility with the legacy BIGQUERY_TO_EXASOL signature
-- and translates calls to the new unified MIGRATE_TO_EXASOL UDF.
-- Note: PROJECT_ID is passed in options as opaque JSON for BigQuery-specific handling.

local options_json = '{"project_id":"' .. (PROJECT_ID or '') .. '"}'

-- Call the new unified migration UDF with BigQuery-specific options
...
table.insert(legacy_result, {
    NULL,
    'FALSE',
    'WARNING: BIGQUERY_TO_EXASOL is deprecated; use MIGRATE_TO_EXASOL(\'bigquery\', ...).'
})
