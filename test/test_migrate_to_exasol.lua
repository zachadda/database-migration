...
test("SQLSERVER metadata template contains STRING_AGG for src_partitions", function()
    ...
    assert_contains(sqlserver_template, "string_agg", "SQLSERVER template should contain string_agg for partition aggregation")
    assert_contains(sqlserver_template, "partition_number", "SQLSERVER template should reference partition_number")
    assert_contains(sqlserver_template, "$partition", "SQLSERVER template should contain $partition predicate syntax")
end)

test("SQLSERVER partitioned table populates src_partitions JSON", function()
    local content = read_file("migrate_to_exasol.sql")
    ...
    assert_contains(sqlserver_section, 'name', "Template should return JSON with name field")
    assert_contains(sqlserver_section, 'predicate', "Template should return JSON with predicate field")
    assert_contains(sqlserver_section, "varchar(2000000)", "Template should use varchar(2000000) for src_partitions")
end)

test("SQLSERVER non-partitioned table emits src_partitions = NULL", function()
    local content = read_file("migrate_to_exasol.sql")
    local sqlserver_section = content:match("SQLSERVER = {.-pair = ")
    ...
    assert_contains(sqlserver_section, "else NULL end", "Template should return NULL when no partitions exist")
end)
...
test("ORACLE template cast-hardens src_partitioned and src_partitions", function()
    ...
    assert_contains(oracle_section, "cast(NULL as varchar(2000000))", "ORACLE template should cast NULL to varchar(2000000) for src_partitions")
end)
...
test("VERTICA template cast-hardens src_partitioned and src_partitions", function()
    ...
    assert_contains(vertica_section, "cast(NULL as varchar(2000000))", "VERTICA template should cast NULL to varchar(2000000) for src_partitions")
end)
...
test("DATABRICKS template cast-hardens src_partitioned and src_partitions", function()
    ...
    assert_contains(databricks_section, "cast(NULL as varchar(2000000))", "DATABRICKS template should cast NULL to varchar(2000000) for src_partitions")
end)
